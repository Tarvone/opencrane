{{- define "opencrane.databaseSchema.job" -}}
{{- if .Values.clustertenantManager.migrationJob.enabled }}
# Reconcile the Prisma schema on EVERY install/upgrade, independent of the OpenCrane
# server pod template. The server also carries a `db-migrate` initContainer, but an
# initContainer only fires when the pod is (re)created — so a `helm upgrade` whose
# server values are unchanged never rolls the pod and never re-runs schema reconciliation.
