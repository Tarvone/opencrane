...
#
# Usage:
#   apps/_infra/deploy-k8s/platform/configure-oidc.sh \
...
# Disable OIDC again with: apps/_infra/deploy-k8s/platform/configure-oidc.sh --disable [--context …]
#
...
if [[ -z "$CHART_DIR" ]]; then
  err "--chart (or OPENCRANE_CHART_DIR) is required. The fleet-platform chart no longer ships in this repo — pass a checked-out copy of WeOwnAI's apps/fleet-platform, or apps/_infra/deploy-k8s for a silo release."
  exit 1
fi
