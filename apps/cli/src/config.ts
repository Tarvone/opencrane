...
{
  /** Full base URL including the /api/v1 suffix. */
  baseUrl: string;
...
interface _CredentialsFile
{
  ...
  token: string;
  /** Raw base URL of the control-plane instance (no /api/v1 suffix). */
  baseUrl?: string;
}
 ...
 *
 * @param opts - Global option object from Commander (url only; token flag removed).
 */
export function _ResolveConfig(opts: { url?: string }): CliConfig
{
  ...
  // 2. Resolve the base URL from flag, env, credentials file, or default.
  const rawUrl =
    opts.url
