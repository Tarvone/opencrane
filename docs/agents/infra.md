...
- **Deploy scripts.** The central fleet chart deploys via `apps/fleet-platform/deploy.sh` and each silo via `apps/clustertenant-platform/deploy.sh`; both drive the shared engine in `libs/k8s-platform/` (`k8s-deploy.sh`, `gke-deploy.sh`, `configure-oidc.sh`, plus `deploy-single-tenant.sh`). Cluster-provisioning installers also live in `libs/k8s-platform/`: `install.sh` (local k3d / `gcp` GKE entrypoint, behind `pnpm install:phase1:*`), `vps-deploy.sh` (provisions k3s on a bare VM), and `deploy-single-tenant.sh` (one org = fleet + one silo). They provision the cluster, then call the fleet role wrapper. (The old `deploy.sh` dispatcher + `wizard.sh` were removed as stale — they only routed to these.) `platform/` no longer exists. Local dev uses k3d value profiles under `libs/k8s-platform/tests/`: `values-k3d-local.yaml` (fast), `-strict.yaml` (prod-like), `-e2e.yaml`.
...
| `libs/k8s-platform/` (Helm library chart + shared deploy engine) | Shared named-templates (`templates/_helpers.tpl`), `k8s-deploy.sh` / `gke-deploy.sh` / `configure-oidc.sh` / `deploy-single-tenant.sh` |
| `apps/fleet-platform/deploy.sh`, `apps/clustertenant-platform/deploy.sh` | Fleet / silo deploy flows |
| `libs/k8s-platform/install.sh`, `vps-deploy.sh`, `deploy-single-tenant.sh` | Cluster-provisioning installers (k3d/GKE, k3s VM, one-org) |
