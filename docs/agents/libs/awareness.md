...
The awareness **contract-version module** used by the opencrane-ui. Org-context RETRIEVAL is no
longer in this package — it moved to the official `@cognee/cognee-openclaw` OpenClaw plugin
(installed into each tenant pod; see `apps/opencrane-api/src/tenants/deploy/2-config-map.ts`
...
[`feat-central-agents`](../apps/feat-central-agents.md) ingests org sources → Cognee. The opencrane-ui
