...
`libs/domain/<domain>/main` package layout (#153). One physical PostgreSQL database and one
...
- `<domain>.prisma` holds the models and enums owned by `libs/domain/<domain>/main`
...
1. **New model/enum → the owning domain's file.** If the owning domain package doesn't
   exist yet, create the lib first (see `libs/domain/README.md`); a model with no owning
