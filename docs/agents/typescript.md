...
- **npm workspaces with integrated NX, pure ESM.** Every package is `"type": "module"`; `tsconfig.base.json` is `module: NodeNext`, `target: es2023`, `strict`. Source-only libs (no package.json); apps in `apps/*` with their own package.json; shared infra/util libs and domain packages under `libs/`, compiled to dist/ via esbuild. NX coordinates task dependencies and caching via `nx.json`. There are 5 apps (`apps/*`), domain packages (`libs/domain/*/main`), shared infra/util libs (`libs/awareness`, `libs/contracts`, `libs/infra-*`, `libs/k8s-platform`, `libs/observability`, `libs/util`); see [`app-specific.md`](./app-specific.md) for the map.
...
- **Frameworks in use** (so the import-order example below reflects reality): opencrane-ui is **Express 5** + Prisma + `@kubernetes/client-node`; the operator is `@kubernetes/client-node` + a custom watch loop; the CLI is a thin typed wrapper over the contracts client. Logging is `pino` everywhere.
- **Types-in-`*.types.ts` is observed with zero known deviations** — e.g. every opencrane-ui route is a `route.ts` + `route.types.ts` pair. Keep it that way.
