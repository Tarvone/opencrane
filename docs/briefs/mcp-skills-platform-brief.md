...
1. **Control plane is the only authority.** Neither plane holds authoritative config. Native admin UIs/APIs are disabled; both are operator-reconciled and drift-detected (reuse `libs/backend/server/projection/main/src/routes/internal/projection-drift.ts` pattern).
...
**Routes** (`libs/backend/server/mcp/main/src/routes/mcp-servers.ts`):
...
**Routes** (`libs/backend/server/skills/main/src/routes/skill-catalog.ts`):
...
**Routes** (`libs/backend/server/retrieval/main/src/routes/third-party-sources.ts`):
