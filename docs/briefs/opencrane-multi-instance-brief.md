...
full OpenCrane (opencrane-ui + operator + Postgres + its own tenant OpenClaw
...
- **The opencrane-ui already targets a configurable namespace** for CR writes
...
- **Ask:** (a) install CRDs **once, cluster-wide**, decoupled from the per-instance
  ...
  **fleet CRD-version compatibility contract** (which opencrane-ui/operator
...
- **Q1** Is anything in the opencrane-ui or operator hardcoded to a fixed namespace
