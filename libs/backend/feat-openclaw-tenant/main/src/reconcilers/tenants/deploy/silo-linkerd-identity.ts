...
import type { LinkerdAuthorizationPolicy, LinkerdMeshTlsAuthentication, LinkerdServer, SiloLinkerdIdentityPolicy } from "./silo-linkerd-identity.types.js";
 ...
 *   opencrane-ui namespace's (the only cross-silo principal) — exactly the S2 ingress
...
{
  // 1. Resolve the shared-plane namespace — the operator/opencrane-ui is the only
  ...
  // 5. The mesh-identity allow-list: intra-silo + the operator/opencrane-ui plane only.
