...
export class CogneeLiteLlmKey
{
   ...
   * its own. Discovered by label (`app.kubernetes.io/component: cognee`, matching the app-owned
   * Cognee Helm unit's labels) rather than by name — the operator cannot compute the chart's
   * release-prefixed Deployment name, and there is exactly one Cognee Deployment per namespace
   * (documented invariant), so label discovery is safe and chart-agnostic.
   *
