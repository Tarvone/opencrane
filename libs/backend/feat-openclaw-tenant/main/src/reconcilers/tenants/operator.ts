...
import { _BuildOrgDomainProvisioner, type OrgDomainProvisioner } from "@opencrane/backend/server/tenancy/cluster-tenants";
...
export class TenantOperator
{
  ...
  async reconcileTenant(tenant: Tenant): Promise<void>
  {
    ...
    try
    {
      ...
      //     from the opencrane-ui internal API. This is a deliberate, non-fatal
      //     operator → opencrane-ui dependency: it never throws. The fetch reports
      ...
      //    opencrane-ui API, and the UI all see the current phase. When the model gate
 ...
 */
export function _CreateTenantOperator(kc: k8s.KubeConfig, config: OpenClawTenantOperatorConfig, hosting: HostingAdapter, baseLog: Logger): TenantOperator
{
  ...
  // 3. Hosting adapter — selected once at the application composition boundary.
