...
This package is part of **Tenancy** — the domain that connects Kubernetes state to a single silo (one tenant's isolated running environment). The
...
(a CR applied outside the API path has no projection row yet). This package owns detecting that
drift and repairing it.

It runs a fail-soft background loop for the silo, reconciling `Tenant` CRDs into the DB, and exposes
the detect/repair primitives its neighbours reuse:

```
 authoritative CRDs (Tenant · AccessPolicy)
        │
...
- `TenantProjectionRepairer` — the background repair loop.
...
Owns no Prisma model of its own. It repairs the `Tenant` read-model (owned by `tenants`) against
its authoritative CRD.
