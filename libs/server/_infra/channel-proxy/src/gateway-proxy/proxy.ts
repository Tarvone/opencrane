...
import type { UpgradeDeps } from "./proxy.types.js";
 ...
 *   1. **Origin allowlist** (CSWSH) — refuse before spending a opencrane-ui call.
 ...
 *      opencrane-ui resolution, and reverse-proxy to `openclaw-<user>.<ns>.svc:<port>`.
 *
...
export async function _HandleUpgrade(deps: UpgradeDeps, req: IncomingMessage, socket: Duplex, head: Buffer): Promise<void>
{
  ...
  //    `opencrane-opencrane-ui`, which would otherwise resolve nothing for a multi-silo owner.
