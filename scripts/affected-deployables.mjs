...
// The tenant image gate builds and exercises two full images; run it only when its inputs move.
const tenantImageChanged = changedFiles.some(function _tenant(file) {
  return file.startsWith("apps/feat-openclaw-tenant/")
    || file === "libs/k8s-platform/tests/tenant-image-immutability.sh"
    || file === ".github/workflows/docker.yml";
});
...
_output("tenant_image_changed", String(tenantImageChanged));
