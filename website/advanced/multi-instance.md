...
**B3**), and the **CRD-version ↔ opencrane-ui/operator compatibility contract** the
...
> (the `oc` CLI, opencrane-ui API, awareness layer) that targets one group.
...
| CRD bundle version | Served apiVersion(s) | Compatible opencrane-ui/operator (`appVersion`) | Status |
|--------------------|----------------------|--------------------------------------------------|--------|
...
2. As the schema evolves, add a row per bundle version and list the `appVersion` range that
   can both read and write it. A opencrane-ui/operator build is "compatible" with a bundle
...
Everything is on the opencrane-ui API (`/api/v1/cluster-tenants`) and mirrored by the CLI —
...
A private vendor (e.g. a hosted-opencrane-ui product) implements that contract in their own
...
`ClusterTenant` CRD but **no** provisioner env on the opencrane-ui Deployment; setting
