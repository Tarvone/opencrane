...
**The opencrane-ui (clustertenant-manager) runs in each silo and serves the tenant-facing API, auth, permissions, and organization data.** This reference documents every Helm configuration key under `clustertenantManager`, with defaults from `apps/opencrane-infra/values.yaml`.
...
| `clustertenantManager.image.tag` | `latest` | Image tag. Pinned at deploy time via `--opencrane-ui-tag` (image tags must be restated per invocation because `--reset-then-reuse-values` re-applies chart defaults for unsupplied keys). |
...
| Key | Default | Purpose |
|-----|---------|---------|
| `clustertenantManager.migrationJob.enabled` | `true` | When true, renders a Job that runs schema migrations before the opencrane-ui pod starts. Reconciles the database schema even when the pod template is unchanged (a plain `helm upgrade` without this would not roll the pod, leaving the schema behind). |
...
Cognee integration for opencrane-ui permission synchronization and tenant-isolation ACLs. Cognee is a required service: the opencrane-ui's permission sync and the awareness retrieval ACL both depend on it.
...
| `clustertenantManager.cognee.endpoint` | `http://cognee:8000` | Cognee HTTP endpoint used by opencrane-ui permission sync routes. When `install` is true, the in-chart Service is reachable at this name. Point it elsewhere only when BYO (`install: false`). |
...
Per-org OIDC session config for the opencrane-ui. Zitadel is the single trusted issuer (Mode-2 broker, no upstream Entra), but any spec-compliant OIDC issuer works. OIDC is off unless `issuerUrl` is set.
...
| `clustertenantManager.oidc.redirectUri` | `""` | Callback URL on the opencrane-ui (e.g. `https://<opencrane-ui-host>/api/v1/auth/callback`). Rendered as `OIDC_REDIRECT_URI`. Must match the registered redirect_uri in your OIDC provider. |
...
Same-origin hosting is the only mode: the opencrane-ui host's Ingress serves the org-admin SPA at `/`, the public API at `/api`, and the OpenClaw gateway WS proxy at `/gateway` (when `gatewayProxy.enabled`) from one origin, so the browser gets first-party cookies with no CORS. Helm owns these rules, so the frontend layer never kubectl-patches the Ingress out-of-band. The legacy `*.<domain>` wildcard Ingress and the bare `/`→opencrane-ui layout were removed once every silo migrated.
