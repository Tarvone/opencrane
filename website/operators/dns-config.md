...
(`<base>`, e.g. `weownai.eu`) and a **fixed super-operator / opencrane-ui host**
...
|------|--------|-----------|--------|
| **Platform wildcard** `*.<base>` (+ apex + opencrane-ui host) | every org host `<org>.<base>`, the apex, the fixed opencrane-ui host | DNS-01 (wildcard requires DNS-01) | once at install by the chart (`cluster-issuer.yaml`) |
...
2. **A cert-manager issuer** — an ACME DNS-01 issuer referencing that Secret (or the raw
   ...
   certificate (plus the apex and the opencrane-ui host) into the Secret the chart
