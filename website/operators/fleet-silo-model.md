...
The ClusterTenant management API, Zitadel-admin routes, and platform-DNS RBAC on the fleet-manager are all gated by `clusterTenantManagement.enabled` (the flag `apps/fleet-platform/deploy.sh` sets to `true`; `apps/opencrane-infra/deploy.sh` sets it to `false`):
