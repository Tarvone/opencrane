...
Projected ServiceAccount tokens are the current in-cluster authentication mechanism for pod-to-opencrane-ui calls. Each tenant pod presents an audience-bound projected token that is validated via the Kubernetes TokenReview API (`/api/internal/contract`, `/api/internal/awareness/participation`). Static bearer tokens remain the automation and break-glass path for operators outside the cluster.
