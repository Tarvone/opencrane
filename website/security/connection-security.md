...
**Envoy/mesh sidecar** over a bespoke opencrane-ui proxy. Option B is a strict
...
4. **Inject + forward.** It strips any client-supplied `X-Forwarded-User`, sets it to
   ...
   only the opencrane-ui-resolved identity.
...
- **Per-session, or a standing audited choke point, is required** → **Option C**
  (opencrane-ui proxy, or a mesh/Envoy sidecar if already on a mesh). Accept the
  stateful-CP weight.
